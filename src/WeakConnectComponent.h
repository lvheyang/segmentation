...
class WeakConnectComponent {
	public :
		WeakConnectComponent( ALConstraintGraph *alg, LanguageModel *l, Constraint c );
	...
	private :
		...
		Constraint weak_relation;

		virtual void dfs(ALConstraintGraph *G , int v, Constraint c) ;
		virtual void dfs_iter(ALConstraintGraph *G , int v, Constraint c ) ;
};
